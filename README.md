...
Si el capítulo trae `tap_animation_name`, un toque en cualquier zona libre de la cámara activa ese clip por nombre exacto; Unity lo reproduce una vez sin desplazamiento añadido y luego vuelve a `Idle`, `Quieto` o `Stand` si existe. Los toques sobre botones, la tarjeta de lectura, el estado o el panel docente no lo activan. Si el clip configurado no está en el GLB, registra una advertencia y usa el comportamiento automático anterior: tocar el modelo busca `Walk`, `Walking` o `Caminar` y lo desplaza 2,5 cm sobre la página; el toque no define su destino. Un OBJ/prefab estático también se desplaza, pero sus patas no se animan: para una hormiga que camina de verdad hay que subir un GLB con rig/huesos o piezas animadas y los clips exportados. La API almacena el archivo, no crea animaciones automáticamente. Conviene que el clip de caminar del modo automático sea un ciclo sobre el sitio, pues el desplazamiento lo hace Unity. La detección del toque sobre el modelo usa un colisionador construido a partir de sus límites visuales.
