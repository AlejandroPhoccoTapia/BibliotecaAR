...
Un toque sobre el modelo activa la interacción. Si el capítulo trae `tap_animation_name`, Unity busca ese clip por nombre exacto, lo reproduce una vez sin desplazamiento añadido y luego vuelve a `Idle`, `Quieto` o `Stand` si existe. Si el nombre no está en el GLB, registra una advertencia y usa el comportamiento automático anterior: busca `Walk`, `Walking` o `Caminar` y desplaza el modelo 2,5 cm sobre la página. Un OBJ/prefab estático también se desplaza, pero sus patas no se animan: para una hormiga que camina de verdad hay que subir un GLB con rig/huesos o piezas animadas y los clips exportados. La API almacena el archivo, no crea animaciones automáticamente. Conviene que el clip de caminar del modo automático sea un ciclo sobre el sitio, pues el desplazamiento lo hace Unity. La detección del toque usa un colisionador construido a partir de los límites visuales del modelo.
